...
### 限制 2：已有嵌套关系不能被改变，但平行关系可以变成嵌套或者调整顺序
...
`head` 和 `user` 原来是平行关系，现在变成嵌套关系；其他元素原来已经有的嵌套关系没有发生变化，是合法的。
...
1. 生成的元素必须存在，不能删除
2. 已有嵌套关系不能被改变，但平行关系可以变成嵌套或者调整顺序变

的条件下，HTML 可以随意修改调整结构和样式，并且又可以保持和代码配置的一致性。

解决了 HTML 和配置的一致性问题之后，修改 HTML 结构问题就得到了解决。到时候甚至可以做 Lint 来保证上述原则不被违反。

## 重新生成的设计

这时候还有另外一个问题：修改了代码里面的配置，HTML 里面的核心结构怎么同步调整和修改（HTML 可能在不违反一致性原则的情况下调整过）？
